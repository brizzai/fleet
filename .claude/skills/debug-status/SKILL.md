...
- **Blind to `/compact`.** Compaction is not counted as `busy`, so the file stays `idle` through a multi-minute compaction — a blind spot it shares with the hook and the transcript. If the symptom is "idle during compaction," this file won't rescue it. The **pane** sees it *conditionally*: `detectRunning` matches the `· Compacting conversation… (2m 27s)` activity line, but only while that line is inside the bottom 20 non-empty lines (`whimsicalN := min(20, len(recentLines))`). During a long compaction, queued messages and chrome rendered beneath it can push it out — so check where the line actually sits before concluding the pane layer is broken.
